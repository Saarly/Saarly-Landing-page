...
import { dateLabel, money, numberValue, row, rows, text, unitLabel, type PortalRow } from "@/components/merchant/portal-utils";
...
export function BuyerRequestsSection({ payload, locale, refresh, notify }: BuyerSectionProps) {
  ...
  async function startVoiceRecording() {
    if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === "undefined") {
      notify(locale === "ar" ? "التسجيل الصوتي غير متاح هنا. ارفع ملفًا صوتيًا بدلًا من ذلك." : "Audio recording is unavailable here. Upload an audio file instead.", "error");
      return;
    }
    try {
      ...
      notify(locale === "ar" ? "تعذر تشغيل الميكروفون. اسمح باستخدامه من إعدادات الجهاز." : "Could not start the microphone. Allow microphone access in your device settings.", "error");
    }
  }
  ...
  return <div className="portal-section-stack">
    ...
    {offerReview ? (() => {
      ...
      const deliveryLabel = text(review.preview.delivery_method) === "weight" && deliveryWeight !== null ? (locale === "ar" ? `تكلفة التوصيل بالوزن (${deliveryWeight} كيلوجرام)` : `Weight delivery (${deliveryWeight} kilograms)`) : (locale === "ar" ? "تكلفة التوصيل" : "Delivery cost");
    ...
    {newOpen ? <div className="portal-modal-backdrop"><section className="portal-modal buyer-request-modal"><header><div><span className="eyebrow"><Icon name="quote"/>{locale === "ar" ? "طلب تسعير جديد" : "New quote request"}</span><h2>{locale === "ar" ? "اكتب أو ارفع قائمة المنتجات" : "Enter or upload your item list"}</h2><p>{locale === "ar" ? "راجع كل عنصر قبل الإرسال؛ الأسعار لا تُستخرج من الفاتورة." : "Review every item before sending; invoice prices are not extracted."}</p></div><button className="icon-button" data-modal-close aria-label={locale === "ar" ? "إغلاق" : "Close"} onClick={() => setNewOpen(false)}><Icon name="close"/></button></header><form className="portal-form" onSubmit={analysisQuoteId ? approveAnalysis : submitNew}>
      <div className="portal-subtabs request-source-tabs">{(["manual","image","pdf","voice"] as NewMode[]).map((source) => <button type="button" className={mode === source ? "active" : ""} key={source} onClick={() => { setMode(source); setAnalysisQuoteId(""); clearVoiceRecording(); setItems([initialItem(locale)]); }}>{source === "manual" ? (locale === "ar" ? "يدوي" : "Manual") : source === "image" ? (locale === "ar" ? "صورة" : "Image") : source === "pdf" ? (locale === "ar" ? "مستند" : "Document") : (locale === "ar" ? "صوت" : "Voice")}</button>)}</div>
      {mode !== "manual" && !analysisQuoteId && mode !== "voice" ? <label className="file-drop-zone"><Icon name="upload" size={28}/><strong>{locale === "ar" ? "اختار الملف" : "Choose file"}</strong><small>{mode === "image" ? (locale === "ar" ? "ملف صورة — حتى 4 ميجابايت" : "Image file — up to 4 megabytes") : (locale === "ar" ? "ملف مستند — حتى 18 ميجابايت" : "Document file — up to 18 megabytes")}</small><input type="file" required accept={mode === "image" ? "image/jpeg,image/png,image/webp" : "application/pdf"} capture={mode === "image" ? "environment" : undefined} onChange={(event) => setFile(event.target.files?.[0] ?? null)}/>{file ? <span>{file.name}</span> : null}</label> : null}
