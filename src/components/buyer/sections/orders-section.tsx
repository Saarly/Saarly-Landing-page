...
export function BuyerOrdersSection({ payload, locale, refresh, notify }: BuyerSectionProps) {
  ...
  async function orderAction(action: string, orderId: string) {
    if (action === "cancel_order" || action === "delete_order") {
      const deleting = action === "delete_order";
      const accepted = await confirm({
        title: deleting ? (locale === "ar" ? "حذف الطلب من السجل" : "Remove order from history") : (locale === "ar" ? "إلغاء الطلب" : "Cancel order"),
        body: deleting ? (locale === "ar" ? "الطلب هيختفي من سجل العرض داخل حسابك." : "The order will be removed from your visible history.") : (locale === "ar" ? "سيتم إلغاء الطلب قبل تأكيد المتجر." : "The order will be cancelled before merchant confirmation."),
        confirmLabel: deleting ? (locale === "ar" ? "حذف من السجل" : "Remove") : (locale === "ar" ? "إلغاء الطلب" : "Cancel order"),
        tone: "danger",
      });
      if (!accepted) return;
    }
        ...
        return <article className="buyer-order-card" key={id} data-record-id={id}>
          ...
          {fulfillments.map((fulfillment) => {
            ...
            return <section className="order-merchant-block" key={text(fulfillment.id)}>
              <div className="order-merchant-head"><div><strong>{text(merchant.store_name, locale === "ar" ? "المتجر" : "Store")}</strong><MerchantTrustBadges founderEnabled={merchant.founder_badge_enabled === true} trustedEnabled={merchant.trusted_badge_enabled === true} locale={locale} compact/><small>{text(merchant.branch_name, locale === "ar" ? "بيانات الفرع غير متاحة" : "Branch details unavailable")}</small></div><StatusBadge value={fulfillment.status} locale={locale}/></div>
              <div className="order-lines">{items.map((item) => <div key={text(item.id)}><span>{text(item.matched_name_snapshot || item.requested_name_snapshot)} × {numberValue(item.quantity_snapshot)} {unitLabel(item.unit_snapshot, locale)}</span><strong>{money(item.line_total_snapshot, currency, locale)}</strong></div>)}</div>
              ...
              <div className="inline-actions">{Number.isFinite(Number(merchant.latitude)) && Number.isFinite(Number(merchant.longitude)) ? <a className="button secondary compact" href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(`${merchant.latitude},${merchant.longitude}`)}`} target="_blank" rel="noopener noreferrer"><Icon name="location"/>{locale === "ar" ? "اتجاهات" : "Directions"}</a> : null}<button className="button secondary compact" disabled={busyId.startsWith(`chat:${id}`)} onClick={() => void openChat(order, merchant)}><Icon name="quote"/>{locale === "ar" ? "المحادثة" : "Chat"}</button><button className="button secondary compact" onClick={() => setReview({ orderId: id, merchantId, storeName: text(merchant.store_name), stars: numberValue(existingReview.stars, 5), comment: text(existingReview.comment) })}><Icon name="check"/>{locale === "ar" ? (text(existingReview.id) ? "تعديل التقييم" : "تقييم المتجر") : (text(existingReview.id) ? "Edit review" : "Review store")}</button></div>
            </section>;
          })}
