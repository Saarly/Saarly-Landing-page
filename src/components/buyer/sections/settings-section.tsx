...
export function BuyerSettingsSection({ payload, locale, refresh, notify }: BuyerSectionProps) {
  ...
  function locate() { if (!navigator.geolocation) {notify(locale==="ar"?"تحديد الموقع غير مدعوم على هذا الجهاز.":"Location access is not supported on this device.","error");return;} navigator.geolocation.getCurrentPosition((position) => { setLatitude(String(position.coords.latitude)); setLongitude(String(position.coords.longitude)); }, () => notify(locale === "ar" ? "تعذر الوصول لموقعك." : "Could not access your location.", "error"), { enableHighAccuracy: true, timeout: 12000 }); }
  ...
  function saveScope(next:string){setScope(next);try{window.localStorage.setItem(SEARCH_SCOPE_KEY,next);}catch{} notify(locale==="ar"?"تم حفظ نطاق البحث الافتراضي على هذا الجهاز.":"Default search scope saved on this device.","success");}
  async function toggleNotifications(){const next=!browserNotifications;if(next&&"Notification" in window&&Notification.permission==="default"){const permission=await Notification.requestPermission();if(permission==="denied"){notify(locale==="ar"?"تم رفض السماح بالإشعارات من إعدادات الجهاز.":"Notification access was denied.","error");return;}}setBrowserNotifications(next);try{window.localStorage.setItem(NOTIFICATION_KEY,String(next));}catch{}notify(locale==="ar"?"تم حفظ إعداد إشعارات الموقع على هذا الجهاز.":"Website notification setting saved on this device.","success");}
  ...
  return <div className="portal-section-stack settings-v2">
    <div className="settings-summary-banner"><span><Icon name="settings" size={24}/></span><div><strong>{locale==="ar"?"إعدادات حساب المشتري":"Buyer account settings"}</strong><p>{locale==="ar"?"الموقع ونطاق البحث واللغة والمظهر والتنبيهات والسياسات في مكان واحد، بنفس منطق التطبيق وبترتيب أوضح للموقع.":"Location, search scope, language, appearance, notifications, and policies in one clear website layout."}</p></div></div>
    ...
    <div className="portal-two-columns"><PortalPanel title={locale==="ar"?"نطاق البحث الافتراضي":"Default search scope"} subtitle={locale==="ar"?"نفس اختيار التطبيق: المدينة أو المحافظة أو كل الدولة. يمكن تغييره داخل كل طلب.":"The same app choices: city, governorate, or whole country. You can override it per request."}><div className="setting-options three"><button type="button" className={scope==="city"?"selected":""} onClick={()=>saveScope("city")}><Icon name="store"/><strong>{locale==="ar"?"مدينتي":"City"}</strong></button><button type="button" className={scope==="governorate"?"selected":""} onClick={()=>saveScope("governorate")}><Icon name="location"/><strong>{locale==="ar"?"محافظتي":"Governorate"}</strong></button><button type="button" className={scope==="country"?"selected":""} onClick={()=>saveScope("country")}><Icon name="globe"/><strong>{locale==="ar"?"كل الدولة":"Whole country"}</strong></button></div></PortalPanel><PortalPanel title={locale==="ar"?"إشعارات الموقع":"Website notifications"} subtitle={locale==="ar"?"هذا الاختيار محفوظ على هذا الجهاز، وسجل إشعارات سعرلي يظل متاحًا دائمًا.":"This setting is saved on this device, and your Saarly notification history remains available."}><div className="settings-switch-row"><span className={browserNotifications?"on":""}><Icon name="bell"/></span><div><strong>{locale==="ar"?"استقبال إشعارات الموقع":"Receive website alerts"}</strong><small>{"Notification" in globalThis&&typeof Notification!=="undefined"?`${locale==="ar"?"السماح بالإشعارات":"Notification access"}: ${notificationPermissionLabel(Notification.permission, locale)}`:(locale==="ar"?"إشعارات النظام غير مدعومة هنا":"System notifications are unavailable here")}</small></div><button type="button" className={`switch-control ${browserNotifications?"on":""}`} aria-pressed={browserNotifications} onClick={()=>void toggleNotifications()}><i/></button></div></PortalPanel></div>
    <PortalPanel title={locale === "ar" ? "اللغة والمظهر" : "Language and appearance"} subtitle={locale === "ar" ? "يتم حفظ الاختيار في حسابك ويُستخدم في التطبيق والموقع." : "These preferences are saved to your account and used across the app and website."}><form className="portal-form" onSubmit={savePreferences}><div className="setting-options"><button type="button" className={language === "ar" ? "selected" : ""} onClick={() => setLanguage("ar")}><Icon name="globe"/><strong>{locale === "ar" ? "العربية" : "Arabic"}</strong></button><button type="button" className={language === "en" ? "selected" : ""} onClick={() => setLanguage("en")}><Icon name="globe"/><strong>{locale === "ar" ? "الإنجليزية" : "English"}</strong></button></div><div className="setting-options"><button type="button" className={theme === "light" ? "selected" : ""} onClick={() => setLocalTheme("light")}><Icon name="sun"/><strong>{locale === "ar" ? "فاتح" : "Light"}</strong></button><button type="button" className={theme === "dark" ? "selected" : ""} onClick={() => setLocalTheme("dark")}><Icon name="moon"/><strong>{locale === "ar" ? "داكن" : "Dark"}</strong></button></div><button className="button primary" disabled={saving === "preferences"}>{saving==="preferences"?(locale==="ar"?"جارٍ الحفظ":"Saving"):(locale === "ar" ? "حفظ التفضيلات" : "Save preferences")}</button></form></PortalPanel>
    <div className="portal-two-columns"><PortalPanel title={locale==="ar"?"قيّم تطبيق سعرلي":"Rate Saarly app"} subtitle={locale==="ar"?"بعد نشر التطبيق رسميًا تقدر تفتحه مباشرة على المتجر المناسب.":"Open the relevant store once the app is officially published."}><div className="store-rating-actions">{googlePlay?<a className="button secondary" href={googlePlay} target="_blank" rel="noopener noreferrer"><Icon name="star"/>{locale==="ar"?"متجر جوجل":"Google Play"}</a>:null}{appStore?<a className="button secondary" href={appStore} target="_blank" rel="noopener noreferrer"><Icon name="star"/>{locale==="ar"?"متجر آبل":"App Store"}</a>:null}{!googlePlay&&!appStore?<Notice tone="info">{locale==="ar"?"روابط التقييم هتتفعّل بعد نشر التطبيق رسميًا.":"Rating links will be enabled after the app is officially published."}</Notice>:null}</div></PortalPanel><PortalPanel title={locale === "ar" ? "الدعم والدعوات" : "Support & invites"}><div className="settings-links"><Link href="/buyer/support"><Icon name="quote"/><span><strong>{locale === "ar" ? "الدعم والمساعدة" : "Support and help"}</strong><small>{locale === "ar" ? "الشكاوى والمحادثات السابقة والجديدة." : "New and previous support conversations."}</small></span><Icon name="chevron"/></Link><Link href="/buyer/referrals"><Icon name="users"/><span><strong>{locale==="ar"?"الدعوات والمكافآت":"Invites and rewards"}</strong><small>{locale==="ar"?"رابط الدعوة والتقدم والمكافآت.":"Invite link, progress, and rewards."}</small></span><Icon name="chevron"/></Link></div></PortalPanel></div>
