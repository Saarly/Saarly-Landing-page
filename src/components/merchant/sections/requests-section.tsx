...
export function RequestsSection({ payload, locale, refresh, notify }: SectionProps) {
  ...
  const visible = useMemo(() => requests.filter((request) => {
    ...
    return filter === "expiring" ? new Date(text(request.expires_at)).getTime() - Date.now() < 24 * 3600000 : items.length > 0;
  }), [requests, filter]);

  function openRequest(request: PortalRow) {
    setSelected(request);
    setAnswers(rows(request.items).map((item) => ({
      rfq_item_id: text(item.id), decision: "priced", unit_price: "", unit: text(item.unit_snapshot, "قطعة"), note: "",
    })));
  }
