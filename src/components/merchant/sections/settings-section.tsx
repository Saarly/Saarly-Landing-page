...
const SOCIALS=[
  {label:"Facebook",href:process.env.NEXT_PUBLIC_FACEBOOK_URL||"https://www.facebook.com/share/1AKE7rcMHb/"},
  {label:"Instagram",href:process.env.NEXT_PUBLIC_INSTAGRAM_URL||"https://www.instagram.com/saarly_1"},
  {label:"TikTok",href:process.env.NEXT_PUBLIC_TIKTOK_URL||"https://tiktok.com/@saarly_1"},
];
...
export function SettingsSection({ payload, locale, notify }: SectionProps) {
  ...
  async function toggleNotifications(){const next=!browserNotifications;if(next&&"Notification" in window&&Notification.permission==="default"){const permission=await Notification.requestPermission();if(permission==="denied"){notify(locale==="ar"?"تم رفض إذن إشعارات المتصفح.":"Browser notification permission was denied.","error");return;}}setBrowserNotifications(next);try{window.localStorage.setItem(NOTIFICATION_KEY,String(next));}catch{}notify(locale==="ar"?"تم حفظ إعداد تنبيهات البوابة.":"Portal notification preference saved.","success");}
  ...
  return <div className="portal-section-stack settings-v2">
    ...
    <div className="portal-two-columns"><PortalPanel title={locale === "ar" ? "الحساب" : "Account"}><div className="detail-list"><div><span>{locale === "ar" ? "البريد" : "Email"}</span><strong>{payload.account.email}</strong></div><div><span>{locale === "ar" ? "الاسم" : "Name"}</span><strong>{text(profile.full_name, "—")}</strong></div><div><span>{locale === "ar" ? "نوع الوصول" : "Access type"}</span><strong>{payload.account.isOwner ? (locale === "ar" ? "صاحب المتجر" : "Store owner") : text(payload.account.staff?.role_label)}</strong></div><div><span>{locale==="ar"?"المتجر":"Store"}</span><strong>{text(payload.account.merchant.store_name,"—")}</strong></div></div></PortalPanel><PortalPanel title={locale==="ar"?"إدارة المتجر المتقدمة":"Advanced store management"}><div className="settings-links"><Link href="/merchant/store"><Icon name="store"/><span><strong>{locale==="ar"?"بيانات المتجر وتوفر الحرفي":"Store details & craftsman availability"}</strong><small>{locale==="ar"?"اسم المتجر والتصنيفات وطريقة التسعير والتوفر.":"Store name, categories, pricing mode, and availability."}</small></span><Icon name="chevron"/></Link><Link href="/merchant/employees"><Icon name="users"/><span><strong>{locale==="ar"?"الموظفون والصلاحيات":"Staff & permissions"}</strong><small>{locale==="ar"?"إدارة الرتب والفروع والصفحات المتاحة لكل موظف.":"Manage roles, branches, and page permissions."}</small></span><Icon name="chevron"/></Link></div></PortalPanel></div>
    <div className="portal-two-columns"><PortalPanel title={locale==="ar"?"إشعارات البوابة":"Portal notifications"} subtitle={locale==="ar"?"الاختيار محفوظ على المتصفح؛ سجل الإشعارات داخل حسابك يفضل متاح دائمًا.":"This preference is stored in the browser; your account notification history remains available."}><div className="settings-switch-row"><span className={browserNotifications?"on":""}><Icon name="bell"/></span><div><strong>{locale==="ar"?"استقبال إشعارات الطلبات والرسائل":"Receive order and message alerts"}</strong><small>{typeof Notification!=="undefined"?`${locale==="ar"?"إذن المتصفح":"Browser permission"}: ${notificationPermissionLabel(Notification.permission, locale)}`:(locale==="ar"?"غير مدعوم على هذا المتصفح":"Not supported by this browser")}</small></div><button type="button" className={`switch-control ${browserNotifications?"on":""}`} aria-pressed={browserNotifications} onClick={()=>void toggleNotifications()}><i/></button></div></PortalPanel><PortalPanel title={locale==="ar"?"حالة الحساب والاشتراك":"Account & subscription"}><div className="settings-links"><Link href="/merchant/account-status"><Icon name="shield"/><span><strong>{locale==="ar"?"حالة الحساب":"Account status"}</strong><small>{locale==="ar"?"الصلاحية واستقبال الطلبات وفترة السماح.":"Access, new-work eligibility, and grace period."}</small></span><Icon name="chevron"/></Link>{payload.account.isOwner?<Link href="/merchant/subscriptions"><Icon name="card"/><span><strong>{locale==="ar"?"الاشتراكات والباقات":"Subscriptions & plans"}</strong><small>{locale==="ar"?"الباقات وطلبات الدفع حسب إعدادات الإدارة الحالية.":"Plans and payment requests driven by current admin settings."}</small></span><Icon name="chevron"/></Link>:null}</div></PortalPanel></div>
    <PortalPanel title={locale === "ar" ? "اللغة والمظهر" : "Language and appearance"} subtitle={locale === "ar" ? "يتم حفظ التفضيلات في حساب سعرلي لتتوافق مع التطبيق." : "Preferences are saved to your Saarly account to stay aligned with the app."}><form className="portal-form" onSubmit={save}><div className="setting-options"><button type="button" className={language === "ar" ? "selected" : ""} onClick={() => setLanguage("ar")}><Icon name="globe"/><strong>العربية</strong></button><button type="button" className={language === "en" ? "selected" : ""} onClick={() => setLanguage("en")}><Icon name="globe"/><strong>English</strong></button></div><div className="setting-options"><button type="button" className={theme === "light" ? "selected" : ""} onClick={() => setTheme("light")}><Icon name="sun"/><strong>{locale === "ar" ? "فاتح" : "Light"}</strong></button><button type="button" className={theme === "dark" ? "selected" : ""} onClick={() => setTheme("dark")}><Icon name="moon"/><strong>{locale === "ar" ? "داكن" : "Dark"}</strong></button></div><div className="form-actions"><button className="button primary" type="submit" disabled={saving}>{saving ? (locale === "ar" ? "جارٍ الحفظ" : "Saving") : (locale === "ar" ? "حفظ التفضيلات" : "Save preferences")}</button></div></form></PortalPanel>
    <div className="portal-two-columns"><PortalPanel title={locale==="ar"?"قيّم تطبيق سعرلي":"Rate Saarly app"}>{googlePlay||appStore?<div className="store-rating-actions">{googlePlay?<a className="button secondary" href={googlePlay} target="_blank" rel="noopener noreferrer"><Icon name="star"/>Google Play</a>:null}{appStore?<a className="button secondary" href={appStore} target="_blank" rel="noopener noreferrer"><Icon name="star"/>App Store</a>:null}</div>:<Notice tone="info">{locale==="ar"?"روابط التقييم هتتفعّل بعد نشر التطبيق رسميًا.":"Rating links will be enabled after official app publication."}</Notice>}</PortalPanel><PortalPanel title={locale === "ar" ? "الدعم" : "Support"}><div className="settings-links"><Link href="/merchant/support"><Icon name="mail"/><span><strong>{locale === "ar" ? "الدعم والمساعدة" : "Support & help"}</strong><small>{locale === "ar" ? "الشكاوى السابقة أو محادثة جديدة مع المساعد وفريق الدعم." : "Previous complaints or a new conversation with the assistant and support team."}</small></span><Icon name="chevron"/></Link><Link href="/merchant/referrals"><Icon name="users"/><span><strong>{locale==="ar"?"الدعوات والمكافآت":"Invites & rewards"}</strong><small>{locale==="ar"?"تابع التقدم والمكافآت من نفس بيانات التطبيق.":"Track progress and rewards from the same app data."}</small></span><Icon name="chevron"/></Link></div></PortalPanel></div>
    <div className="portal-two-columns"><PortalPanel title={locale === "ar" ? "السياسات" : "Policies"}><div className="settings-links"><Link href="/privacy"><Icon name="shield"/><span><strong>{locale === "ar" ? "سياسة الخصوصية" : "Privacy policy"}</strong><small>{locale === "ar" ? "تعرف على استخدام البيانات." : "Learn how data is used."}</small></span><Icon name="chevron"/></Link><Link href="/terms"><Icon name="receipt"/><span><strong>{locale === "ar" ? "الشروط والأحكام" : "Terms and conditions"}</strong></span><Icon name="chevron"/></Link><Link href="/refund-policy"><Icon name="history"/><span><strong>{locale==="ar"?"سياسة الإرجاع واسترداد المبالغ":"Returns and refunds policy"}</strong></span><Icon name="chevron"/></Link></div></PortalPanel><PortalPanel title={locale==="ar"?"تابع سعرلي":"Follow Saarly"}><div className="social-link-grid">{SOCIALS.map(item=><a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer"><Icon name="globe"/><strong>{item.label}</strong><Icon name="arrow" size={16}/></a>)}</div></PortalPanel></div>
