...
export function EmployeesSection({ payload, locale, refresh, notify }: SectionProps) {
  ...
  function openEditor(employee?: ReturnType<typeof row>) {
    ...
    setForm({ email: text(employee.email), roleLabel: text(employee.role_label), permissions: { ...defaultPermissions, ...mergedPermissions }, branchIds: Array.isArray(employee.branch_ids) ? employee.branch_ids.map((item) => text(item)).filter(Boolean) : [] });
    setOpen(true);
  }
  ...
  async function remove(id: string) {
    if (!(await confirm({ title: locale === "ar" ? "إيقاف وصول الموظف" : "Disable staff access", body: locale === "ar" ? "الموظف مش هيقدر يدخل لمساحة المتجر لحد ما تضيفه أو تفعّله من جديد." : "This staff member will lose access to the store workspace until enabled again.", confirmLabel: locale === "ar" ? "إيقاف الوصول" : "Disable access", tone: "danger" }))) return;
