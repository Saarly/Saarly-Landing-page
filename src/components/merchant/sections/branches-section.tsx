...
import { bool, localizedSystemText, money, numberValue, rows, text, unitLabel, type PortalRow } from "@/components/merchant/portal-utils";
...
export function BranchesSection({ payload, locale, refresh, notify }: SectionProps) {
  ...
  const cityById = useMemo(() => new Map(cities.map((city) => [text(city.id), city])), [cities]);
  const branchLocation = (branch: PortalRow) => {
    const city = cityById.get(text(branch.city_id));
    if (!city) return locale === "ar" ? "الموقع غير محدد" : "Location not set";
    const cityName = text(locale === "ar" ? city.name_ar : city.name_en);
    const governorate = text(locale === "ar" ? city.governorate_ar : city.governorate_en);
    return [governorate, cityName].filter(Boolean).join(" · ") || (locale === "ar" ? "الموقع غير محدد" : "Location not set");
  };
  ...
  async function submit(event: FormEvent) {
    ...
    if (!form.id && (!form.managerIdFrontPath || !form.managerIdBackPath)) {
      notify(locale === "ar" ? "ارفع وجه وظهر بطاقة مدير الفرع." : "Upload both sides of the branch manager identity card.", "error"); return;
    }
    if ((form.managerIdFrontPath && !form.managerIdBackPath) || (!form.managerIdFrontPath && form.managerIdBackPath)) {
      notify(locale === "ar" ? "يجب رفع الوجه والظهر معًا." : "Both sides of the identity card must be uploaded together.", "error"); return;
    }
  ...
  return <div className="portal-section-stack">
    ...
    <PortalPanel title={locale === "ar" ? "فروع المتجر" : "Store branches"} subtitle={locale === "ar" ? "الموقع والمستندات ومدير الفرع وخدماته." : "Location, documents, manager, and services."} action={isOwner ? <button className="button primary compact" type="button" onClick={() => edit()}><Icon name="plus" size={18}/>{locale === "ar" ? "إضافة فرع" : "Add branch"}</button> : undefined}>
      {branches.length === 0 ? <EmptyState icon="branch" title={locale === "ar" ? "لا توجد فروع" : "No branches"} body={locale === "ar" ? "أضف الفرع الأول مع مستندات مديره." : "Add the first branch with its manager documents."} action={isOwner ? <button className="button primary compact" type="button" onClick={() => edit()}>{locale === "ar" ? "إضافة فرع" : "Add branch"}</button> : undefined}/> : <div className="branch-grid">{branches.map((branch) => {
        ...
        return <article className="branch-card" key={branchId} data-record-id={branchId}>
          ...
          <header><span className="branch-icon"><Icon name="branch"/></span><div><h3>{text(branch.name)}</h3><p>{branchLocation(branch)}</p></div><StatusBadge value={branch.approval_status} locale={locale}/></header>
          ...
          {text(branch.rejection_reason) ? <p className="inline-error">{localizedSystemText(branch.rejection_reason, locale, locale === "ar" ? "راجع سبب الرفض مع الدعم." : "Contact support for the rejection details.")}</p> : null}
          {isOwner && branchDocs.some((doc) => text(doc.rejection_reason)) ? <p className="inline-error">{locale === "ar" ? `سبب رفض المستندات: ${localizedSystemText(branchDocs.find((doc) => text(doc.rejection_reason))?.rejection_reason, locale, "راجع التفاصيل مع الدعم.")}` : `Document rejection reason: ${localizedSystemText(branchDocs.find((doc) => text(doc.rejection_reason))?.rejection_reason, locale, "Contact support for details.")}`}</p> : null}
          {isOwner && branchDocs.some((doc) => text(doc.preview_signed_url)) ? <div className="branch-document-preview-actions">{branchDocs.filter((doc) => text(doc.preview_signed_url)).map((doc) => <a className="button text-button compact" key={text(doc.id)} href={text(doc.preview_signed_url)} target="_blank" rel="noreferrer"><Icon name="eye" size={16}/>{text(doc.kind) === "branch_manager_id_front" ? (locale === "ar" ? "معاينة وجه بطاقة المدير" : "Preview identity card front") : text(doc.kind) === "branch_manager_id_back" ? (locale === "ar" ? "معاينة ظهر بطاقة المدير" : "Preview identity card back") : text(doc.kind) === "commercial_register" ? (locale === "ar" ? "معاينة السجل التجاري" : "Preview commercial register") : (locale === "ar" ? "معاينة المستند" : "Preview document")}</a>)}</div> : null}
    ...
    {open && isOwner ? <div className="portal-modal-backdrop" role="presentation"><section className="portal-modal wide" role="dialog" aria-modal="true"><header><div><span className="eyebrow"><Icon name="branch" size={17}/>{form.id ? (locale === "ar" ? "تعديل فرع" : "Edit branch") : (locale === "ar" ? "فرع جديد" : "New branch")}</span><h2>{locale === "ar" ? "بيانات الفرع ومديره" : "Branch and manager details"}</h2></div><button className="icon-button" data-modal-close type="button" aria-label={locale === "ar" ? "إغلاق" : "Close"} onClick={() => setOpen(false)}><Icon name="close"/></button></header>
      <form className="portal-form" onSubmit={submit}>
        ...
        <div className="branch-upload-grid"><FileUpload title={locale === "ar" ? "صورة واجهة الفرع" : "Branch storefront photo"} accept="image/jpeg,image/png,image/webp" uploaded={Boolean(form.frontImageUrl)} busy={uploading === "front"} preview={form.frontPreviewUrl} locale={locale} onFile={(file) => void upload("front", file)}/><FileUpload title={locale === "ar" ? "وجه بطاقة المدير" : "Manager identity card front"} accept="image/jpeg,image/png,image/webp" uploaded={Boolean(form.managerIdFrontPath)} busy={uploading === "managerFront"} preview={form.managerIdFrontPreviewUrl} locale={locale} onFile={(file) => void upload("managerFront", file)}/><FileUpload title={locale === "ar" ? "ظهر بطاقة المدير" : "Manager identity card back"} accept="image/jpeg,image/png,image/webp" uploaded={Boolean(form.managerIdBackPath)} busy={uploading === "managerBack"} preview={form.managerIdBackPreviewUrl} locale={locale} onFile={(file) => void upload("managerBack", file)}/></div>
