...
export function ProductsSection({ payload, locale, refresh, notify }: SectionProps) {
  ...
  async function submit(event: FormEvent) {
    ...
    setSaving(true);
    try {
      await portalPost("save_product", { ...form, imageUrl: form.imageUrls[0] ?? "", price, quantity, shippingWeightKg: form.deliveryPricingMethod === "weight" ? shippingWeight : null, weightInKg: form.weightInKg ? Number(form.weightInKg) : null });
