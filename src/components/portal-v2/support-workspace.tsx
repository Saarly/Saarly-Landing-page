...
export function SupportWorkspace({ data, locale, post, notify }: Props) {
  ...
  async function closeConversation() {
    ...
    if (!(await confirm({
      title: locale === "ar" ? "إنهاء المحادثة" : "End conversation",
      body: locale === "ar" ? "لو عايز تخرج بس، تقدر تنتقل لأي صفحة والمحادثة هتفضل مفتوحة. اضغط إنهاء فقط لو المشكلة خلصت." : "If you only want to leave, navigate to any page and the conversation will stay open. End it only when the issue is resolved.",
      confirmLabel: locale === "ar" ? "إنهاء المحادثة" : "End conversation",
      cancelLabel: locale === "ar" ? "خليها مفتوحة" : "Keep open",
      tone: "danger",
    }))) return;
    setBusy("close");
    try {
