...
function errorResponse(error: unknown) {
  ...
  return NextResponse.json({ error: error instanceof Error ? error.message : "registration_request_failed" }, { status: 500 });
}

export async function GET(request: NextRequest) {
  try {
    const { user, userDb, service } = await requireAuthenticatedUser(request);
    const [{ data: profile, error: profileError }, { data: merchant, error: merchantError }, locations, categories] = await Promise.all([
      service.from("users").select("id,full_name,mobile,primary_email,recovery_email,role,preferred_language,theme,is_blocked").eq("id", user.id).maybeSingle(),
      service.from("merchants").select("*, merchant_categories(category_id,is_primary), branches(*), merchant_documents(kind,storage_bucket,storage_path,status,rejection_reason)").eq("user_id", user.id).eq("is_archived", false).order("created_at", { ascending: false }).limit(1).maybeSingle(),
      userDb.rpc("app_location_options"),
      service.from("categories").select("id,name_ar,name_en,parent_id,display_order,is_active").eq("is_active", true).order("display_order").order("name_ar"),
    ]);
    ...
    if (locations.error) throw new PortalError("location_options_load_failed", 500);
    if (categories.error) throw new PortalError("categories_load_failed", 500);
    return NextResponse.json({ data: { user: { id: user.id, email: user.email ?? "" }, profile, merchant, locations: locations.data ?? [], categories: categories.data ?? [] } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "submit_registration") {
      ...
      const { data, error } = await userDb.rpc("submit_my_merchant_registration", { p_payload: payload });
      if (error) throw new PortalError(error.message, 400);
      return NextResponse.json({ data });
    }
